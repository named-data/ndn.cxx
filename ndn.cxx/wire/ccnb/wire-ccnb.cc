...
size_t
Ccnb::AppendNumber (OutputIterator &start, uint32_t number)
{
...
// size_t
// Ccnb::AppendTimestampBlob (OutputIterator &start, const Time &time)
// {
...
size_t
Ccnb::AppendTimestampBlob (OutputIterator &os, const TimeInterval &time)
{
...
size_t
Ccnb::EstimateTimestampBlob (const TimeInterval &time)
{
  int required_bytes = 2; // 12 bits for fractions of a second, 4 bits left for seconds. Sometimes it is enough
  intmax_t ts = time.total_seconds () >> 4;
  for (;  required_bytes < 7 && ts != 0; ts >>= 8) // not more than 6 bytes?
...
size_t
Ccnb::AppendTaggedBlobWithPadding (OutputIterator &start, uint32_t dtag,
                                   uint32_t length,
                                   const uint8_t *data, size_t size)
{
  if (size > length)
...
size_t
Ccnb::SerializeName (OutputIterator &start, const Name &name)
{
  size_t written = 0;
...
Ptr<Name>
Ccnb::DeserializeName (InputIterator &i)
{
