...
namespace security
{
  ...
  bool 
  IdentityPolicy::satisfy (const Data & data)
  {
  ...
  bool 
  IdentityPolicy::satisfy (const Name & dataName, const Name & signerName)
  {
  ...
  bool 
  IdentityPolicy::matchDataName (const Data & data)
  {
    return m_dataNameRegex.match(data.getName ());
  }

  bool
  IdentityPolicy::matchSignerName (const Data & data)
  {    
    DigestAlgorithm digestAlg = DIGEST_SHA256; //For temporary, should be assigned by Signature.getAlgorithm();
    KeyType keyType = KEY_TYPE_RSA; //For temporary, should be assigned by Publickey.getKeyType();
    if(KEY_TYPE_RSA == keyType && DIGEST_SHA256 == digestAlg)
  ...
  bool 
  IdentityPolicy::compare(const Name & dataName, const Name & signerName)
  {
